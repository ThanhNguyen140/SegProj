...
class TrainEpoch:
    ...
    def train_epoch(self):
        ...
        for epoch in range(self.start_epoch, self.epoch_num):
            ...
            for idx, batch in tqdm(enumerate(self.train_dataset)):
                train_loss, train_iou, train_dice, train_pre, train_recall = (
                    self._run_train(batch)
                )
                train_avg_loss += train_loss.item()
                train_avg_iou += train_iou.item()
                train_avg_dice.append += train_dice.item()
                train_avg_pre += train_pre.item()
                train_avg_recall += train_recall.item()

            train_avg_loss = train_avg_loss / idx
            train_avg_dice = train_avg_dice / idx
            train_avg_iou = train_avg_iou / idx
            train_avg_pre = train_avg_pre / idx
            train_avg_recall = train_avg_recall / idx
            ...
            for idx, batch in tqdm(enumerate(self.val_dataset)):
                val_loss, val_iou, val_dice, val_pre, val_recall, _, _ = self._run_val(
                    batch
                )
            ...
            val_avg_loss = val_avg_loss / idx
            val_avg_dice = val_avg_dice / idx
            val_avg_iou = val_avg_iou / idx
            val_avg_pre = val_avg_pre / idx
            val_avg_recall = val_avg_recall / idx
            val_avg_pre = val_avg_pre / idx
            ...
            if epoch % self.report == 0:
                ...
                self.save_info(epoch)

            if early_stopping.check_stop(val_avg_loss):
    ...
    def _run_train(self, batch):
        ...
        with torch.amp.autocast("cuda", enabled=True):
            ...
            pred = torch(probs > 0.5, 1, 0)
            loss = dice_loss(labels, probs)
            loss.backward()
    ...
    def save_epoch(self, epoch):
        ...
        torch.save(checkpoint, f"{self.output_path}/checkpoint{epoch}.pth")
