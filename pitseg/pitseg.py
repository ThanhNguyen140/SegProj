...
ckpt_path = '../pitseg/checkpoint/pitseg_0.1.0.pt'
...
class Segmentor:
    def __init__(self, model, x_center = 88, y_center = 135, z_center = 69):
        self.model = model
    ...
    def patching(self,imgs_tensor, w = 16):
        self.xmin, self.xmax = int(self.x_center - w), int(self.x_center + w)
        self.ymin, self.ymax = int(self.y_center - w), int(self.y_center + w)
        self.zmin, self.zmax = int(self.z_center - w), int(self.z_center + w)
    ...
    def predict(self, imgs, threshold = 0.5):
        ...
        for idx in range(new_centroids.shape[0]):
            ...
            if (self.x_center < 16) or (self.y_center < 16) or (self.z_center < 16):
                self.reset()
    ...
    def postprocess(self, logits, threshold):
        ...
        pred = RemoveSmallObjects(min_size=100)(pred)
